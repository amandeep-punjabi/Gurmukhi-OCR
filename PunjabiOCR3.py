...
def process_user_images(image_path):
    if image_path.endswith(('.gif', '.png', '.jpg', '.jpeg', '.jpg')):
        ...
        # Use pytesseract with custom configuration to extract both Punjabi and English
        extracted_text = pytesseract.image_to_string(image, lang='pan+eng', config='--psm 6 --oem 1')
...
def process_user_pdfs(pdf_path):
    if pdf_path.endswith('.pdf'):
        ...
        # Use pytesseract with custom configuration to extract both Punjabi and English
        extracted_text = pytesseract.image_to_string(pdf_text, lang='pan+eng', config='--psm 6 --oem 1')
